...
# wpca #
wPCA <- ade4::withinpca(select(ft, gmin, RWC, FvFm, LA, LT, LDMC, CC, Ptlp, SLA, LSWC), as.factor(ft$SpeciesLong), 
                  scannf = F, nf = 5)
...
```{r pcaArticle}
g.among <- autoplot(princomp(~ gmin + RWC + FvFm + LA + LT + LDMC + CC + Ptlp + SLA + LSWC, data = ft, cor = T), 
         data = ft,
...
```{r indpcadbh}
autoplot(princomp(~ gmin + RWC + FvFm + LA + LT + LDMC + CC + Ptlp + SLA + LSWC, data = ft, cor = T), 
         data = ft,
