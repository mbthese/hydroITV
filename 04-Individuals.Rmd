...
```{r , eval=F, echo=T}
g <- read_csv2("data/Paracou_P16_2020.csv") %>% 
  filter(Plot == 16, SubPlot == 10) %>% 
   mutate(label = paste0("P", Plot, "-", SubPlot, "-", TreeFieldNum)) %>% 
  ggplot(aes(Xfield, Yfield, label = label)) +
  geom_point()
```
...
`r sum(missings$inds)` individuals remaining.
...
g <- ggplot(candidates, aes(col = paste(Genus, Species), shape = TypeTopo)) +
  geom_sf()
plotly::ggplotly(g)
