...
# traits %>%
#   group_by(trait, Ind, Sample) %>% 
#   summarise(value = mean(value, na.rm = T)) %>% 
#   reshape2::dcast(Ind + Sample ~ trait, value.var = "value") %>% 
#   write_tsv( "~/Téléchargements/IndFT.tsv")
```
...
traits %>% 
  ...
  sample_n(4) %>% 
  group_by(trait) %>% 
  do(var = nlme::lme(value ~ 1, random=~1|Ind/Sample, data = .) %>% 
  ...
  mutate(trait = recode(trait, "Ptlp" = "Psi[TLP]", "FvFm" = "frac(Fv,Fm)")) %>% 
  mutate(level = factor(level, levels = c("Individual", "Sample", "Residual"))) %>% 
  ...
  scale_fill_manual(expression(sigma), 
                    values = unname(varcols[c("individual", "sample", "residual")])) +
